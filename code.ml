...
let neighbors cells cell =
  filter (fun c -> mem (point_of c) (point_neighbors (point_of cell))) cells

let next_cell cells cell =
  match cell with
  | Alive _ -> if mem (length (filter is_alive (neighbors cells cell))) [2; 3]
    then Alive (point_of cell)
    else Dead (point_of cell)
  | Dead _ -> if (length (filter is_alive (neighbors cells cell))) = 3
    then Alive (point_of cell)
    else Dead (point_of cell)
